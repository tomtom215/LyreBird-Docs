# License

LyreBirdAudio is licensed under the Apache License 2.0.

---

## Apache License 2.0

A permissive free software license written by the Apache Software Foundation.

## Key Points

- YES Commercial use allowed
- YES Modification allowed
- YES Distribution allowed
- YES Patent use allowed
- YES Private use allowed
- WARNING: License and copyright notice required
- WARNING: State changes required
- NO Trademark use not allowed
- NO Liability limitation
- NO Warranty disclaimer

---

## Full License Text

The complete Apache License 2.0 text is available in the [LICENSE file](https://github.com/tomtom215/LyreBirdAudio/blob/main/LICENSE) in the repository.

---

## Third-Party Components

LyreBirdAudio uses the following open source components:

- **MediaMTX** - MIT License
- **FFmpeg** - LGPL 2.1 or GPL 2.0 (depending on build)

---

[Main Documentation](../index.md) | [GitHub Repository](https://github.com/tomtom215/LyreBirdAudio)

---

## Get Started

Ready to use LyreBirdAudio?

[Quick Start Guide →](../getting-started/quick-start.md){ .md-button .md-button--primary }
[Installation Guide →](../getting-started/installation.md){ .md-button }
